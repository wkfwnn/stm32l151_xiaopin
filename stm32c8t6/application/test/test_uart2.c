...
void test_uart2_function(void const * argument)
{
	...
	if(sc == RET_OK){
		DBG_LOG("test_uart2 get %s handle is %d\n","uart2",test_uart2_handle);
	}else{
		DBG_LOG("test_uart2 get %s handle fail %d,exit test_uart1_task \n","uart2",sc);
		vTaskDelete(NULL);
	}
	while(1){
		#if 0
		...
		if(sc == RET_OK){
			DBG_LOG("test_uart2 transmit ok\n");
		}else{
...
void test_uart2_func()
{
	...
	if(test_uart2_handle == NULL){
		DBG_LOG("test_uart2_function create fail\n");
	}else{
		DBG_LOG("test_uart2_function create success\n");
	}
}
